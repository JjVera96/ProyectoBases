...
<html>
<head>
	<title>insertar Producto - Droguerias FERJO</title>
	<meta charset="utf-8">
...
<div id='Contenido'>
	...
	<section>
		...
		<form action="actualizarMedicamento.php" method="post">
			...
			<br><input type="submit" value="Actualizar Datos" name="btnSubmit" id="btnSubmit"><br>
		</form>
		</div>
	</section>
</div>
<?php
	...
	if(isset($_POST["btnSubmit"])){
		if(isset($_POST["NombreMed"]) and isset($_POST["NombreFab"]) and isset($_POST["Presentacion"]) and isset($_POST["Tipo"]) and    isset($_POST["fechaVen"]) and isset($_POST["Precio"]) and isset($_POST["codigo"]) and isset($_POST["codProv"]) and          isset($_POST["formula"]) and isset($_POST["Disp"])){
			...
			$consulta = "UPDATE drogas SET Nombre='$nompro', Fabricante='$nomfab', Presentacion='$prepro', Tipo='$tipopro', Fecha_Vencimiento='$fevenpro', Precio='$preciopro', Disponibilidad='$dispro', Cod_Proveedor='$codprov', Formula='$forpro' WHERE Codigo='$codpro'";
		...
		if($result==false){
			...
			echo "<h1 id='Bien'>Registro de producto actualizado</h1>";
		}
	}
